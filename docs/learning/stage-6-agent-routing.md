...
阶段六之后不要直接跳到 RAG，也不要马上做复杂多 Agent。更合理的路线是先把业务能力工具化，再接入 LLM，最后再让 LLM 参与单 Agent 编排。
...
阶段 6：规则路由版 Agent 入口
  ...
  -> 阶段 12：RAG 检索闭环
  -> 阶段 13：测试补齐
  -> 阶段 14：Docker Compose 与本地启动
  -> 阶段 15：代码清理与文档收尾
```

各阶段边界如下：

- 阶段 7 只做 Tool 标准化，不要求接入 LLM。
- 阶段 8 接入 LLM，但先让 LLM 做意图识别、参数抽取和结果总结，不让 LLM 直接执行写操作。
- 阶段 9 才进入单 Agent 编排，让 LLM 参与工具选择和多轮决策，但工具执行仍由代码校验和调用。
- 阶段 10 增强上下文，支持缺参追问、pending action 和指代消解。
- 阶段 11 和阶段 12 再补知识构建与 RAG。RAG 只服务历史经验检索，不替代当前工单事实查询。

这条路线的核心原则是：LLM 负责理解、规划和总结；Tool 负责受控执行；`biz` 负责最终业务规则和事实状态。

后续 Agent 开发还应遵循统一的工程设计原则，详见 `docs/learning/agent-engineering-principles.md`。其中重点包括 Tool-first、受控执行、结构化上下文、高风险操作确认、执行轨迹和失败恢复。
